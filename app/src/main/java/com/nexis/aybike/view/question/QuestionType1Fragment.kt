...
class QuestionType1Fragment(val questionData: Question, val qIn: Int, val qSize: Int) : Fragment(), View.OnClickListener {
    ...
    override fun onClick(p0: View?) {
        p0?.let {
            when (it.id){
                R.id.question_type1_fragment_txtAnswer1 -> Singleton.setNextQuestionPage(qSize)
                R.id.question_type1_fragment_txtAnswer2 -> Singleton.setNextQuestionPage(qSize)
                R.id.question_type1_fragment_txtAnswer3 -> Singleton.setNextQuestionPage(qSize)
                R.id.question_type1_fragment_txtAnswer4 -> Singleton.setNextQuestionPage(qSize)
            }
        }
    }
